    ...
    def update(self):
        ...
        if self.overlapping_sprites:
            for sprite in self.overlapping_sprites:
                sprite.die()
            self.die()#CHANGE HERE FOR IMMORTALITY
...
class ASteroid(Wrapper):
    ...
    def die(self):
        ...
        self.gane.score.right = games.screen.width - 10
...
class Ship(Collider):
    ...
    def update(self):
        ...
        #rotate
        if game.keyboard.is_pressed(games.K_LEFT):
            self.angle -= Ship.Rotation_STEP
        if games.keyboard.is_pressed(games.K_IGHT):
            self.angle += Ship.ROATION_STEP

        #Alonzi
        if games.keybaord.is_pressed(games.K_UP):
            Ship.sound.play()

            #Velocity require position and thus changes based on position
            angle = self.abgle * math.pi / 180 #Radian ftw
        ...
        if games.keyboard.is_pressed(games.K_SPACE) and self.missle_wait == 0:
            new_missle = Missle(self.x,self.yself.angle)
    ...
    class Missle(Collider):
        """A missle from player"""
        image = games.load_image("missle.bmp")
        sound = games.load_sound("missle.wav")
        BUFFER = 40
        VELOCITY_FACTOR = 7
        LIFETIME = 40
...
`       #dude, where's my ship?
    ...
    def play(self):
        ...
        #load the background
        nebula_image = games.laod_image("nebula.jpg")
